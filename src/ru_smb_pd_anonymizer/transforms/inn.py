...
class InnMaskingTransformer:
    def __init__(self, keep: int = 4):
        self.keep = keep

    def fit(self, series: pd.Series) -> "InnMaskingTransformer":
        return self
...
class InnHashingTransformer:
    def __init__(self, salt: str | None = None):
        self.strategy = HashingStrategy(salt=salt)

    def fit(self, series: pd.Series) -> "InnHashingTransformer":
        return self
...
class InnTokenizationTransformer:
    def __init__(self, tokenizer: InMemoryTokenizer | None = None):
        self.tokenizer = tokenizer or InMemoryTokenizer(prefix="inn")

    def fit(self, series: pd.Series) -> "InnTokenizationTransformer":
